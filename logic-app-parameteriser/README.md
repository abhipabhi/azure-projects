# Azure Logic App ARM Template Parameteriser
...
A utility for converting exported **Azure Logic App JSON definitions** into **environment-agnostic, parameterised ARM templates** suitable for deployment across multiple Azure environments.

This repository provides a CLI wrapper that executes a transformation notebook to automatically parameterise Logic App templates.
...
- Automatic extraction of environment-specific values
- Generation of reusable ARM templates
- Batch processing of Logic App exports
...
│
├── examples/
│   ├── original-logic-app.json
│   └── parameterised-template.json
│
├── requirements.txt
└── README.md
```
...
| File | Description |
|-----|-------------|
| `converter.ipynb` | Notebook containing the Logic App parameterisation logic |
| `parameteriser.py` | CLI wrapper used to execute the notebook against JSON files |
...
```
python3 parameteriser.py <input-folder>
```

Example:

```
python3 parameteriser.py Desktop/Input
```
...
Each Logic App file is converted into a **parameterised ARM template**.
...
3. For each Logic App file:
   ...
   - The Logic App definition is parameterised
...
Logic Apps that rely on **Azure Custom Connectors (`Microsoft.Web/customApis`)** cannot be fully parameterised by this tool.
...
- Support for custom connector parameterisation
...
- Security teams exporting Azure Sentinel playbooks
- DevOps teams managing Logic App deployments across environments
- Infrastructure teams maintaining reusable ARM templates
